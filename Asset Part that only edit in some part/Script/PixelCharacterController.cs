...
namespace Cainos.CustomizablePixelCharacter
{
    public class PixelCharacterController : MonoBehaviour
    {
        ...
        {
            GameObject.Find("GameManager").GetComponent<GameManagerScript>().CurrentTakeAction = CharacterID;
            //GameObject.Find("GameManager").GetComponent<GameManagerScript>().CheckAction();
        }
        ...
        private void Update()
        {
            if (PermissionToMove)
            {
                ...
                if(inputH == 0) IsWalking = false;

                if (IsWalking && !SFXManager.GetComponent<AudioSource>().isPlaying && GetComponent<PixelCharacter>().IsGrounded) SFXManager.Playsound(0);
                else if (!IsWalking) SFXManager.GetComponent<AudioSource>().Stop();
        ...
        public void Move(float inputH, bool inputCrouch, bool inputRunning, bool inputJump)
        {
            ...
            if (isGrounded)
            {
             ...
             /*   if (acc == walkAcc && fx.MovingBlend > 0) SFXManager.Playsound(0);
                if (acc == runAcc && fx.MovingBlend > 0) SFXManager.Playsound(5);*/

                if (inputCrouch)
                {
                    acc = crouchAcc;
                    max = crouchSpeedMax;
                    gameObject.GetComponent<CapsuleCollider2D>().size = new Vector2(0.489498138f, 1.19309163f);                   
                }
                else if (!inputCrouch) gameObject.GetComponent<CapsuleCollider2D>().size = new Vector2(0.489498138f, 1.87826824f);
            }
            else
            {
                acc = airAcc;
