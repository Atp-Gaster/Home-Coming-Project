...
namespace Cainos.CustomizablePixelCharacter
{
    ...
    public class PixelCharacter : MonoBehaviour
    {
        ...
        #region RUNTIME
        public bool EntryCrouchArea = false;
        ...
        public void Attack()
        {
            if(!CooldownAttack)
            {
                ...
                var fireScript = GetComponent<CharacterFireBullet>();
                fireScript.Launch();
                if (gameObject.name == "Main Character") StartCoroutine(PlaySFX(3));
                else StartCoroutine(PlaySFX(2));
                coroutine = CDButton();
        ...
        {            
            if (!Audio.GetComponent<AudioSource>().isPlaying) Audio.GetComponent<AudioManager>().Playsound(index);
        ...
        private void Update()
        {
            ...
            if (Input.GetKeyDown(KeyCode.W) && IsCrouching && !EntryCrouchArea)
            {
                IsCrouching = false;
                transform.position = new Vector2(transform.position.x, transform.position.y + 0.1f);
            }

            /*if (movingBlend >= 0.1f) StartCoroutine(PlaySFX(0));*/
        }
